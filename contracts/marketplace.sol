...
contract AJMarketplace is ReentrancyGuard {
    ...
    function getBidderAddress(uint256 _nftId, uint8 _auctionIndex)
        public
        view
    ...
    function bid(uint256 _nftId, uint256 _payAmount) external {
        ...
        (bool result, ) = isAddressInArray(
            allBidersAddress[_nftId][auctionIndex],
            msg.sender
        );
    ...
    function transferToHigherBidder(uint256 _nftId) external {
        ...
        if (AuctionDataset[_nftId].highestBidder != address(0)) {
            ...
            for (
                uint256 i = 0;
                i < allBidersAddress[_nftId][auctionIndex].length;
                i++
            ) {
                if (
                    allBidersAddress[_nftId][auctionIndex][i] !=
                    AuctionDataset[_nftId].highestBidder
                ) {
                    // Transfer
                    address userAddress = allBidersAddress[_nftId][
                        auctionIndex
                    ][i];
                    IWETH(WETH).transfer(
                        allBidersAddress[_nftId][auctionIndex][i],
                    ...
                    uint256 etherValue_platform = (NftPrice * (platformFee)) /
                        100;
                    uint256 etherValue_royalty = (NftPrice * (royaltyPercent)) /
                        100;
